...
def log_action(logger, action, args, result):
    ...
    if isinstance(result, str) and result != "exit":
        ...
        short_result = result.split('\n')[0][:50]  # Перший рядок, макс. 50 символів
        if len(result.split('\n')[0]) > 50:
            short_result += "..."
        ...
        try:
            ...
            if timestamp_end > 0:
                ...
                if action_end > 0:
                    ...
                    formatted_logs.append(f"║ {'Дата та час:':<14} {timestamp:<84} ║")
                    formatted_logs.append(f"║ {'Дія:':<14} {action:<84} ║")
                    ...
                    if detail_lines:
                        formatted_logs.append(f"║ {'Деталі:':<14} {detail_lines[0]:<84} ║")
                        for i in range(1, len(detail_lines)):
                            formatted_logs.append(f"║ {'':<14} {detail_lines[i]:<84} ║")
                    else:
                        formatted_logs.append(f"║ {'Деталі:':<14} {'':<84} ║")
                else:
...
def clear_logs(args, logger):
    """Очищає файл логів."""
    confirmation = input("Ви впевнені, що хочете видалити ВСІ записи логу? (y/n): ").lower()
    if confirmation == 'y':
        try:
